...
export function OrderProvider({ children }) {
  ...
  const getAllOrders = async () => {
    ...
    try {
      ...
      const response = await axios.get("https://tala-store.vercel.app/order/user/orders", config)
  ...
  const createOrder = async (orderData) => {
    ...
    try {
      ...
      const response = await axios.post("https://tala-store.vercel.app/order", formData, config)
  ...
  const calculateOrderTotal = (products, city) => {
    // Calculate subtotal from products
    const subtotal = products.reduce((total, item) => {
      const price = Number.parseFloat(item.product?.finalPrice) || 0
    ...
    let shippingFee = 0
    if (city) {
      if (city.includes("Cairo")) {
        shippingFee = 80
      } else if (city.includes("Giza")) {
        shippingFee = 50
      }
    }

    // Return total (subtotal + shipping fee)
